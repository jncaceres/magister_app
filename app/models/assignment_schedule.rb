class AssignmentSchedule < ActiveRecord::Base
  ...
  #after_create :send_schedule_notification
  ...
  def send_schedule_notification
    ...
    if self.stage == "Primera"
      User.with_role(:student).each do |u|
      ...
      @users = User.eager_load(attachments: [assignment_schedule: :assignment])
        .where("number = ? AND assignment_schedules.stage = ?",
          assignment_number,
          AssignmentSchedule.stages["Primera"])
        .with_role(:student)
      @users.each do |u|
      ...
      @users = User.eager_load(reverse_corrections: [assignment_schedule: :assignment])
        .where("number = ? AND assignment_schedules.stage = ? AND attachment_id IS NOT NULL",
          assignment_number,
