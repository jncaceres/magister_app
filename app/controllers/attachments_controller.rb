...
#------------------ UPLOAD DOWNLOAD CONTROLLERS ------------------------
  ...
  def upload_file
    ...
    if Time.now.asctime > @schedule.end_date || @schedule.stage != "Primera"
  ...
  def upload_second_stage_correction
    if params[:attachment][:essay].nil? || params[:puntaje].nil?
  ...
  def upload_third_stage_correction
    ...
    if !@correction.nil?
      ...
      if @correction.save
        ...
        redirect_back fallback_location: root_path, alert: 'Hubo un error en subir tu corrección: ' + @correction.errors.full_messages.to_s + "\n Si sigues con problemas escribe a preguntas@ensayosoyce.com."
      end
    else
      redirect_back fallback_location: root_path, alert: "Hubo un error en subir tu corrección, por favor inténtalo nuevamente. Si sigues con problemas escribe a preguntas@ensayosoyce.com."
    end
  end
  ...
  def ajax_download
    ...
    if !@attachment.nil?
      if @attachment.user_id == current_user.id || current_user.has_role?(:admin)
        ...
        if !correction.nil? && @schedule.stage != "Primera"
          ...
          redirect_back fallback_location: root_path, alert: 'Hubo un error al intentar descargar tu archivo, si sigues con problemas envía un mail a preguntas@ensayosoyce.com.'
        end
      end
    end
  end
  ...
  private
    ...
    def attachment_params
      params.require(:attachment).permit(:user_id, :essay, :name, :assignment_schedule)
    end
end
