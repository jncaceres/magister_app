class CommentsController < ApplicationController
  ...
  # GET /comments.json
  def index
    @comments = Comment.includes(:video).order(:created_at).all
  end
  ...
  private
    ...
    def set_comment
      @comment = Comment.includes(:children).find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def comment_params
      params.require(:comment).permit(:video_id, :user_id, :parent_id, :content)
    end

    def set_breadcrumbs
      @breadcrumbs = %w(hello world)
    end
end
