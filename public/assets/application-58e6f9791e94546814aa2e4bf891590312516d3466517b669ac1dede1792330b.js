...
riot.tag2('video-tag', '<div id="player" ref="player"></div>', '', '', function(opts) {
    this.player = null;
...
riot.tag2('percentage-box', '<div class="{correct: (value >= 66), incorrect: (value <= 33), yellow: (value > 33 && value < 66)}" if="{value > 0}">{value}%</div> <div style="color: #555;" if="{value == 0}">--</div><div>val: {value}</div>', '', '', function(opts) {
    this.value = opts.value * 100;
});
riot.tag2('dyn-report', '<table class="table"> <thead> <tr> <th></th> <th>Primera Parte</th> <th colspan="6">Segunda Parte</th> </tr> <tr> <th>Video</th> <th>Contenido</th> <th>Interpretar</th> <th>Análisis</th> <th>Evaluación</th> <th>Inferencia</th> <th>Explicación</th> <th>Autoregulación</th> <th>N</th> </tr> </thead> <tbody> <tr each="{trees}"> <td onclick="{selects(id)}">{text}</td> <td class="clickable" onclick="{selects(id)}">{percent(score.content)}</td> <td class="clickable" onclick="{select_filter(id, \'Interpretación\')}">{percent(score.interpretation)}</td> <td class="clickable" onclick="{select_filter(id, \'Análisis\')}">{percent(score.analysis)}</td> <td class="clickable" onclick="{select_filter(id, \'Evaluación\')}">{percent(score.evaluation)}</td> <td class="clickable" onclick="{select_filter(id, \'Inferencia\')}">{percent(score.inference)}</td> <td class="clickable" onclick="{select_filter(id, \'Explicación\')}">{percent(score.explanation)}</td> <td class="clickable" onclick="{select_filter(id, \'Autoregulación\')}">{percent(score.selfregulation)}</td> <td>{score.n}</td> </tr> <tr> <td>Total</td> <td each="{averages}">{percent(this.avg)}</td> </tr> </tbody> </table> <hr> <h3>{tree_title}</h3> <question-show each="{detail.questions}" data="{this}"></question-show>', 'dyn-report .switch,[data-is="dyn-report"] .switch{ position: relative; display: inline-block; width: 60px; height: 34px; } dyn-report .switch input,[data-is="dyn-report"] .switch input{display:none;} dyn-report .slider,[data-is="dyn-report"] .slider{ position: absolute; cursor: pointer; top: 0; left: 0; right: 0; bottom: 0; background-color: #ccc; -webkit-transition: .4s; transition: .4s; } dyn-report .slider:before,[data-is="dyn-report"] .slider:before{ position: absolute; content: ""; height: 26px; width: 26px; left: 4px; bottom: 4px; background-color: white; -webkit-transition: .4s; transition: .4s; } dyn-report input:checked + .slider,[data-is="dyn-report"] input:checked + .slider{ background-color: #2196F3; } dyn-report input:focus + .slider,[data-is="dyn-report"] input:focus + .slider{ box-shadow: 0 0 1px #2196F3; } dyn-report input:checked + .slider:before,[data-is="dyn-report"] input:checked + .slider:before{ -webkit-transform: translateX(26px); -ms-transform: translateX(26px); transform: translateX(26px); } dyn-report .slider.round,[data-is="dyn-report"] .slider.round{ border-radius: 34px; } dyn-report .slider.round:before,[data-is="dyn-report"] .slider.round:before{ border-radius: 50%; }', '', function(opts) {
    ...
    this.select_filter = (id, skill) => {
      return () => {
        $.ajax({
          url: "/courses/" + course_id + "/trees/" + id + ".json",
          success: (payload) => {
            var filtered = payload.tree.questions.filter((question) => question.skills.indexOf(skill) > -1);
...
riot.tag2('comment-form', '<a href="#" if="{!active}" onclick="{activate}">Responder</a> <form onsubmit="{send_data}" if="{active}"> <div class="form-group"> <textarea class="form-control" ref="content" placeholder="Escribe tu respuesta" rows="1" cols="40" onclick="{expand}" onblur="{collapse}"></textarea> <input type="submit" class="btn btn-primary buttonify" value="Enviar"> </div> </form> <br>', '', '', function(opts) {
    this.active = false;
    this.expand = () => {
...
riot.tag2('question-show', '<h4 class="title"><strong>{type}</strong> <span ref="question"></span> <small class="tag">{skills.join()}</small></h4> <div each="{content.choices}" class="container {correct: right}"> <div class="col-md-6">{text}</div> <div class="col-md-6">{n}</div> </div> <h5>{ct.question}</h5> <div each="{ct.choices}" class="container {correct: right}"> <div class="col-md-6">{text}</div> <div class="col-md-6">{n}</div> </div>', '', '', function(opts) {
    this.on('mount', () => {
...
riot.tag2('timer-tag', '<span>{title}: {format(current)}</span>', '', '', function(opts) {
